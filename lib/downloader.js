...
async function ytget(query) {
    ...
    try {
        ...
        $('.tab-item-data').each((index, tab) => {
            ...
            $(tab).find('tbody tr').each((_, element) => {
...
async function brad(text) {
    try {
        // Kirim data ke endpoint generator
        const response = await axios.post(
            'https://www.bratgenerator.com/generate', // Ganti URL jika ada perubahan
            { text: text }, // Data dikirim sesuai form
            { headers: { 'Content-Type': 'application/json' } }
        );

        // Periksa apakah permintaan berhasil
        if (response.status === 200) {
            const imageUrl = response.data.image; // Ubah sesuai struktur respons
            const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });

            // Simpan gambar sebagai file PNG
            const filePath = './brad_image.png';
            fs.writeFileSync(filePath, imageResponse.data);
            console.log(`Gambar berhasil dibuat: ${filePath}`);
            return filePath;
        } else {
            console.error('Gagal membuat gambar Brad:', response.status);
        }
    } catch (error) {
        console.error('Terjadi kesalahan:', error.message);
    }
}
module.exports = { pinterest, pinterest2, wallpaper, remini, wikimedia, quotesAnime, multiDownload, yanzGpt, happymod, mediafireDl, umma, ringtone, jadwalsholat, styletext, tiktokDl, facebookDl, instaStory, bk9Ai, ytMp4, ytMp3, quotedLyo, simi, bigconv, ytget,brad }
